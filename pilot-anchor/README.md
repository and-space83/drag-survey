...
- **② 価値・選好** (`?block=value`、Stage 2。評定ON): trial 直後の主観的確信度 (自信・迷い・難しさ) を軌道から予測できるかの本筋。文字2択で価値差を段階化。

**公開URL**: ① <https://and-space83.github.io/drag-survey/pilot-anchor/> / ② <https://and-space83.github.io/drag-survey/pilot-anchor/?block=value>
...
主観評定 (自信・迷い・難しさ) は **① アンカーでは取得しない**方針（タスクの性質上、確信度は構成的に変動しないため）。評定は **② 価値・選好 (`?block=value`) で有効化**され、各試行直後に3項目×5段階を提示する (`CONFIG.ratings`)。③道徳ジレンマ等は今後追加予定。
...
5. 完了直後に「直前どう持って・どの指で操作したか」を1つ選択 (事後アンケート、5択)
6. **データは自動送信**される (「✅ 送信完了」表示)。参加者の追加操作は不要
...
| 用途 | URL | 備考 |
|---|---|---|
| **① アンカー 本番 (Stage 1)** | `https://and-space83.github.io/drag-survey/pilot-anchor/` | 練習 6 + 本番 84 試行、評定OFF、自動送信 ON |
| **② 価値・選好 本番 (Stage 2)** | `…/pilot-anchor/?block=value` | 練習 2 + 本番 12 試行、**評定ON**、自動送信 ON |
| 短縮スモークテスト (①) | `…/pilot-anchor/?reps=1&practice=0` | 12 試行のみ。**送信は ON なので Drive に届く**点に注意 |
| 短縮スモークテスト (②) | `…/pilot-anchor/?block=value&reps=1&practice=1` | 練習1+本番12。**送信 ON** |
...
| パラメータ | 既定 | 内容 |
|---|---|---|
| `?block=` | anchor | 実行ブロック (`anchor`=①数値サイズ / `value`=②価値・選好)。既定値は各ブロックで変わる |
| `?reps=N` | ①7 / ②1 | 反復数 (①12セル×7=84 / ②12項目×各1回=12。reps を増やすと②は項目を複数回・両側に出す) |
| `?practice=N` | ①6 / ②2 | 練習試行数 |
| `?break=N` | 0 | N 試行ごとの休憩画面 (0 で無効) |
| `?ratings=1` | ①0 / ②1 | 試行後の 3 項目評定 (②は既定ON、①は既定OFF)。`?ratings=0` で無効化も可 |
| `?rotate=0` | — | 横向きガードを無効化 |
...
- **試行構成**: 12 項目 (価値差3 × ドメイン4) を各1回 = **本番 12 試行** + 練習 2。左右CB は項目間で均等 (高価値側を6左6右、水準内2L/2R)。同一設問の2回提示はしない。
- **評定 (ON)**: 各試行の選択直後に 3 項目 × 5 段階 (自信 / 迷い / 難しさ)。
