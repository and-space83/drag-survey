...
修論研究「モバイル入力ダイナミクスから調査回答の確信度を推定する」の実験アプリ群。各アプリは静的ファイルのみで完結し (pilot-anchor は index.html + app.css + bank.js の3ファイル)、サーバ依存はない。研究全体のドキュメント・解析計画・先行研究調査は別リポ (private) で管理。
